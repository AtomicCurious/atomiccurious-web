...
// app/es/(sections)/recursos/recursos-client.tsx
import {
  useCallback,
  useEffect,
  useId,
...
function getResourceWhy(resource: PreparedResource<Resource>) {
  if (
    hasExactTag(resource, "productividad") ||
    resource.normalizedCategory === "productividad"
  ) {
    return "Útil para crear mejores sistemas, enfocar mejor y trabajar con más claridad."
  }

  if (
    hasExactTag(resource, "aprendizaje") ||
    resource.normalizedCategory === "aprendizaje"
  ) {
    return "Útil para entender ideas complejas con más claridad y estructura."
  }

  if (
    hasExactTag(resource, "creatividad") ||
    resource.normalizedCategory === "creativo"
  ) {
    return "Útil para convertir la curiosidad en ideas, proyectos y mejor pensamiento."
  }

  if (
    hasExactTag(resource, "herramienta") ||
    hasExactTag(resource, "herramientas") ||
    resource.normalizedCategory === "herramienta"
  ) {
    return "Útil cuando quieres ventaja práctica, no solo más información."
  }

  if (
    hasExactTag(resource, "libro") ||
    hasExactTag(resource, "libros") ||
    resource.normalizedCategory === "libro"
  ) {
    return "Útil para ir más profundo de lo que suele permitir el contenido corto."
  }

  return "Seleccionado por su claridad, utilidad y valor para una curiosidad a largo plazo."
}
...
function FeaturedResourceCard({
  ...
  return (
    <Link
      ...
      target="_blank"
      rel="noreferrer"
      aria-label={`Abrir recurso destacado: ${resource.title}`}
      className={[
      ...
      <div className="relative flex min-h-[126px] gap-4">
        <div className="ac-res-icon-tile relative flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-border/70 bg-bg/35">
          {resource.image ? (
            <Image
              src={resource.image}
              alt={`Portada o vista previa de ${resource.title}`}
              fill
              sizes="96px"
...
function RecentResourceRow({
  ...
  return (
    <Link
      ...
      target="_blank"
      rel="noreferrer"
      aria-label={`Abrir recurso reciente: ${resource.title}`}
      className={[
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-surface-1/35 p-6 text-center shadow-soft backdrop-blur">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted">
        ...
        {activeCategory !== "Todo" ? ` dentro de ${activeCategory}` : ""}.
        Prueba una búsqueda más amplia o limpia los filtros actuales.
      </p>
...
export default function RecursosClient() {
  ...
  const isFiltering =
    debouncedSearchQuery.trim().length > 0 || activeCategory !== "Todo"
        ...
        <header className="grid items-center gap-10 lg:grid-cols-[1fr_0.9fr]">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 max-w-2xl text-balance text-base leading-relaxed text-muted sm:text-lg">
              Libros, apps, sitios web y herramientas seleccionadas para
              expandir tu perspectiva, afinar tu pensamiento y convertir la
              curiosidad en acción útil.
            </p>
            ...
            <div className="mt-7 max-w-2xl">
              ...
              >
                Buscar recursos de AtomicCurious
              </label>
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={`Mostrar recursos de ${category}`}
                    >
                      {category}
                    </button>
                  )
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <p className="text-sm text-muted" aria-live="polite">
                  {totalResults === 1
                    ...
                    : `${totalResults} recursos encontrados.`}
                </p>
          ...
          <div className="relative hidden min-h-[560px] lg:block">
            ...
            <div className="ac-res-float relative -top-4 h-[560px] w-full scale-[1.08] transition duration-700 hover:scale-[1.11]">
              <Image
                src={HERO_CORE_IMAGE}
                alt="Core leyendo recursos de AtomicCurious"
                fill
                priority
            ...
            <div className="absolute bottom-0 left-1/2 w-[78%] -translate-x-1/2 rounded-2xl border border-[rgb(var(--accent)/0.18)] bg-bg/40 p-3.5 text-center shadow-[0_18px_54px_rgba(var(--accent),0.10)] backdrop-blur-md">
              <p className="text-sm font-semibold text-text">
                Sistema de recomendación de Core
              </p>
              <p className="mt-1 text-xs leading-relaxed text-muted">
                Recursos seleccionados por utilidad, claridad y valor para una
                curiosidad a largo plazo.
              </p>
            </div>
          </div>
        </header>
          ...
          <>
            <section className="mt-10">
              <div className="mb-4 flex items-center justify-between gap-4">
                ...
                <p className="hidden text-xs font-medium text-muted sm:block">
                  Selecciones de alto valor para explorar con más profundidad.
                </p>
              </div>
              ...
              {filteredFeaturedResources.length > 0 ? (
                ...
                <div className="rounded-2xl border border-border/70 bg-surface-1/35 p-6 text-sm text-muted">
                  No hay recursos destacados que coincidan con tus filtros actuales.
                </div>
              )}
            </section>
            ...
            <section className="mt-8">
              <div className="mb-4 flex items-center justify-between gap-4">
                ...
                <p className="hidden text-xs font-medium text-muted sm:block">
                  Nuevas adiciones a la biblioteca de AtomicCurious.
                </p>
              </div>
              ...
              {filteredRecentResources.length > 0 ? (
                ...
                <div className="rounded-2xl border border-border/70 bg-surface-1/35 p-6 text-sm text-muted">
                  No hay recursos recientes que coincidan con tus filtros actuales.
                </div>
              )}
            </section>
          </>
          ...
          <div className="grid items-center gap-6 md:grid-cols-[auto_1fr_auto]">
            ...
            <div className="text-center md:text-left">
              <h2 className="text-2xl font-semibold tracking-tight text-[rgb(var(--accent))]">
                ¿Conoces un gran recurso?
              </h2>

              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
                Sugiere un libro, herramienta, sitio web, curso o sistema que te
                haya ayudado a aprender, pensar, crear o entender algo mejor.
              </p>
            </div>
